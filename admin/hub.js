/* ============================================================
 * Lost Pines Creative — Command Center (owner-only)
 * One front door for every studio surface: cross-domain executive summary,
 * deep links into each console, and the company asset register (merged from
 * the old /ops page). Email+password auth, gated on dd_is_admin. Shares the
 * DeSmit/Groundwork/onboarding Supabase project — sign in here and you're
 * signed in everywhere on this origin.
 * ============================================================ */
...
async function enterDash() {
  ...
  $("greeting").textContent = first ? "Signed in as " + user.email : "";
  await Promise.all([loadSummary(), load()]);
  subscribe();
}

/* ---------- executive summary (cross-domain counts) ---------- */
...
async function loadSummary() {
  const [ddOpen, ddQuote, ddLeads, gwActive, gwOnb, gwMsgs, gwLeads, onbPipe, onbSubmit] = await Promise.all([
    cnt("dd_projects", (q) => q.not("status", "in", "(complete,cancelled)")),
    cnt("dd_projects", (q) => q.eq("status", "requested")),
    cnt("dd_inquiries", (q) => q.eq("status", "new")),
    cnt("gw_clients", (q) => q.eq("status", "active")),
    cnt("gw_clients", (q) => q.eq("status", "onboarding")),
    cnt("gw_messages", (q) => q.eq("status", "new")),
    cnt("gw_inquiries", (q) => q.eq("status", "new")),
    cnt("onb_intakes", (q) => q.in("status", ["submitted", "in_review"])),
    cnt("onb_intakes", (q) => q.eq("status", "submitted")),
  ]);

  setKpi("dd", ddOpen, "open project" + (ddOpen === 1 ? "" : "s"),
    [part(ddQuote, "awaiting quote"), part(ddLeads, "new lead" + (ddLeads === 1 ? "" : "s"))]);
  const gwInbox = gwMsgs + gwLeads;
  setKpi("gw", gwActive, "active client" + (gwActive === 1 ? "" : "s"),
    [part(gwOnb, "onboarding"), part(gwInbox, "in inbox")]);
  setKpi("onb", onbPipe, "in pipeline",
    [part(onbSubmit, "submitted"), part(onbPipe - onbSubmit, "in review")]);
  // Ops tile is filled by renderStats() once assets load.
}

function part(n, label) { return { n, label }; }
function setKpi(key, big, bigLabel, subs) {
  $("k-" + key).textContent = big;
  $("k-" + key).nextElementSibling.innerHTML =
    (bigLabel ? '<span style="color:var(--text-dim)">' + esc(bigLabel) + "</span>" : "") +
    subs.filter((s) => s.n > 0).map((s) => ' · <span class="hot">' + s.n + "</span> " + esc(s.label)).join("");
}

/* ---------- assets (merged from Ops) ---------- */
async function load() {
...
function renderStats() {
  ...
  // mirror into the executive summary Ops tile
  $("k-ops").textContent = "$" + monthly.toFixed(0);
  $("k-ops").nextElementSibling.innerHTML =
    '<span style="color:var(--text-dim)">/mo est.</span>' +
    (soon > 0 ? ' · <span class="hot">' + soon + "</span> renew &le; 30d" : "");
}
...
function subscribe() {
  if (channel) return;
  channel = sb.channel("lpc-hub")
    .on("postgres_changes", { event: "*", schema: "public", table: "lpc_assets" }, load)
    .on("postgres_changes", { event: "*", schema: "public", table: "dd_projects" }, loadSummary)
    .on("postgres_changes", { event: "*", schema: "public", table: "dd_inquiries" }, loadSummary)
    .on("postgres_changes", { event: "*", schema: "public", table: "gw_messages" }, loadSummary)
    .on("postgres_changes", { event: "*", schema: "public", table: "onb_intakes" }, loadSummary)
    .subscribe();
}
